...
const AdminDesigns = () => {
  ...
  return (
    <div className='min-h-screen bg-zinc-50 dark:bg-zinc-900 p-8 flex flex-col items-center justify-center'>
      <SEO
        title="Figma Designs Admin | Ferdous"
        description="Private Figma designs admin."
        path="/admin/add-design"
        noindex
      />
      <div className='w-full max-w-4xl bg-white dark:bg-zinc-800 rounded-xl shadow-lg p-8'>
        <h1 className='text-3xl font-bold mb-6 text-zinc-900 dark:text-white'>Figma Designs Admin</h1>
        <p className='text-zinc-600 dark:text-zinc-400 mb-8'>Upload Figma UI/UX portfolio pieces. Graphic and logo portfolios have their own admin pages.</p>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
          ...
          <div>
            <label className='block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1'>Design Image {editingId && '(Leave empty to keep current)'}</label>
            <input
              type='file'
              accept='image/*'
        ...
        <h2 className='text-2xl font-bold mb-6 text-zinc-900 dark:text-white'>Manage Uploaded Designs</h2>
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
          {projects.length === 0 ? (
            <p className='text-zinc-500'>No designs found.</p>
          ) : (
            projects.map((proj) => (
