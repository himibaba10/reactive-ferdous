...
import { useAdminGraphicPortfolio } from '../hooks/useAdminGraphicPortfolio';
import SEO from './SEO';

const AdminGraphicPortfolio = () => {
  const {
    formData,
    selectedFile,
    setSelectedFile,
    ...
    handleDelete,
    handleSubmit,
  } = useAdminGraphicPortfolio();

  return (
    <div className='min-h-screen bg-zinc-50 dark:bg-zinc-900 p-8 flex flex-col items-center justify-center'>
      <SEO
        title='Graphic Portfolio Admin | Ferdous'
        description='Private graphic design portfolio admin.'
      ...
      <div className='w-full max-w-4xl bg-white dark:bg-zinc-800 rounded-xl shadow-lg p-8'>
        <h1 className='text-3xl font-bold mb-6 text-zinc-900 dark:text-white'>
          Graphic Design Portfolio Admin
        </h1>
        <p className='text-zinc-600 dark:text-zinc-400 mb-8'>
          Add graphic design work that appears on the Graphic Design service page.
        </p>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
          <div>
            ...
            <input
              ...
              placeholder='e.g. Event Poster Series'
            />
          </div>

          <div>
            <label className='block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1'>
              Design Image {editingId && '(Leave empty to keep current)'}
            </label>
            <input
              type='file'
              accept='image/*'
        ...
        <h2 className='text-2xl font-bold mb-6 text-zinc-900 dark:text-white'>
          Manage Graphic Portfolio
        </h2>
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
          {projects.length === 0 ? (
            <p className='text-zinc-500'>No graphic portfolio items yet.</p>
          ) : (
            projects.map((proj) => (
              <div
                key={proj.id}
              ...
              >
                <img
                  src={proj.img}
                  alt={proj.title}
                  className='h-48 w-full object-cover bg-zinc-200 dark:bg-zinc-800'
                />
...
export default AdminGraphicPortfolio;
